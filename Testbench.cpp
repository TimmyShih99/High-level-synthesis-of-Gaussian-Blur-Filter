...
void Testbench::feed_rgb() {
	...
	int adjustX, adjustY,  yBound;//xBound
	n_txn = 0;
	...
	for (y = 0; y != height; ++y) {
		for (x = 0; x != width; ++x) {
			...
			for (v = -yBound; v != yBound + adjustY; ++v) {   //-1, 0, 1
				unsigned int u = 0;
				//for (u = -xBound; u != xBound + adjustX; ++u) { // change to one col
			...
			//std::cout << "x=" << y << " (fed)" <<std::endl;
		}
		//std::cout << "y=" << y << " (fed)" <<std::endl;
	}
	while(true){o_rgb.put(0);}
}
...
void Testbench::fetch_result() {
	...
	for (y = 0; y != height; ++y) {
		for (x = 0; x != width; ++x) {
			...
			total = i_result.read();
#endif
			//int result = (int)(std::sqrt(total)); //Improve coding styles
			sc_dt::sc_uint<8> result = (int)((total)/273);

			
			
			if(x==0) {
				*(target_bitmap + bytes_per_pixel * (width * y + 254) + 0) = result;
				*(target_bitmap + bytes_per_pixel * (width * y + 254) + 1) = result;
				*(target_bitmap + bytes_per_pixel * (width * y + 254) + 2) = result;
			}
			else if(x==1){
				*(target_bitmap + bytes_per_pixel * (width * y + 255) + 0) = result;
				*(target_bitmap + bytes_per_pixel * (width * y + 255) + 1) = result;
				*(target_bitmap + bytes_per_pixel * (width * y + 255) + 2) = result;
			}
			else{
			...
			//std::cout << "x=" << y << " (fetched)" <<std::endl;
		}
		//cout<<"y="<<y<< " (fetched)" <<std::endl;
	}
	cout<<"computing the time elapsed"<<endl;
	total_run_time = sc_time_stamp() - total_start_time;
	cout<<"calling sc_stop"<<endl;
	sc_stop();
}
